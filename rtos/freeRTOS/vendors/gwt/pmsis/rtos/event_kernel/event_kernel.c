...
{
    *wrap = pi_malloc(sizeof(struct pmsis_event_kernel_wrap));
    ...
    struct pmsis_event_kernel *priv = pi_malloc(sizeof(struct pmsis_event_kernel));
    ...
    struct pmsis_event_scheduler *sched = pi_malloc(sizeof(struct pmsis_event_scheduler));
    if(!sched)
    {
        return -1;
    }
